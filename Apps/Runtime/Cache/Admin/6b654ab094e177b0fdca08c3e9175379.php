...
<form method="post" action="/ssyp/index.php/Admin/Admin/re_pwd" onsubmit="return form_submit(this)">
<table id="detail">
	<tr>
		<th colspan="2" class="det_title" >密码修改 (不修改密码，不用填写以下内容)
	...
	<tr >
		<th>重复新密码</th>
		<td>
			<input type="password" name="re_password"  placeholder="重新输入密码" />
		</td>
	</tr>
...
<form method="post" action="/ssyp/index.php/Admin/Admin/re_name" onsubmit="return form_submit(this)">
<table id="detail">
	<tr>
		<th colspan="2" class="det_title" >基本信息
		<input type="hidden" name="re_url" value="self">
		</th>
	</tr>
	<tr>
		<th>真实姓名</th>
		<td>
			<input type="text" name="realname" placeholder="真实姓名" value="<?php echo ($data["realname"]); ?>"/>
		</td>
	</tr>
<!-- 	<tr>
		<th>联系电话</th>
		<td>
			<input type="text" name="phone"  placeholder="联系电话" value="<?php echo ($data["phone"]); ?>" />
...
function dx_send(val){
	var phone = $(val).prev().val();
	alert(phone);
	$.ajax({
		'url':'/ssyp/index.php/Admin/Admin/dxSend/phone/'+phone
	})
