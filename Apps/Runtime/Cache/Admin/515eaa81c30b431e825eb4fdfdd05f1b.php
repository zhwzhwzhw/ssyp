...
<block name="main">
<br>
<a href="/ssyp/index.php/Admin/Role/add/role_type/1">创建后台角色</a>
<br>
<table id="detail">
	<tr>
		<th>角色名</th>
...
<?php if(is_array($role)): foreach($role as $key=>$v): ?><tr>
		...
		<td>
		<ul>
			<?php if(is_array($v["power"])): foreach($v["power"] as $key=>$pid): if(is_array($power)): foreach($power as $key=>$pv): if(($pid) == $pv["id"]): ?><li><?php echo ($pv["power_name"]); ?>：<?php echo ($pv["module"]); ?>/<?php echo ($pv["controller"]); ?>/<?php echo ($pv["action"]); ?></li><?php endif; endforeach; endif; endforeach; endif; ?>
		</ul>
		</td>
		<td>
			<a href="/ssyp/index.php/Admin/Role/add/role_type/<?php echo ($v["role_type"]); ?>/id/<?php echo ($v["id"]); ?>">编辑</a>
		</td>
