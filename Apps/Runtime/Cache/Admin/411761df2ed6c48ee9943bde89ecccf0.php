...
<link href="/ssyp/Public/form_new_style/style.css" type="text/css" rel="stylesheet"/>
<style>
.right_wrap{
...
<form method="post" action="/ssyp/index.php/Admin/User/savetask">
<input type="hidden" name="id" value="<?php echo ($task["t_id"]); ?>">
<input type="hidden" name="userid" value="<?php echo ($userid); ?>">
           <div class="container_form" >
               <div id="container">
               <div class="breadcrumbs">
                   <a  style="color: #4C8FBD" href="/ssyp/index.php/Admin/User/index">团队列表</a>
            ...
            <!--表单-->
              <!-- <div class="form-group">
                   ...
                   <select name="bus_address" class="form-control">
                       <?php if(is_array($area)): foreach($area as $key=>$v): if($v["a_name"] == $data[bus_address]): ?><option value="<?php echo ($v["a_name"]); ?>" selected ><?php echo ($v["a_name"]); ?></option>
                           <?php else: ?>
                           <option value="<?php echo ($v["a_name"]); ?>"><?php echo ($v["a_name"]); ?></option><?php endif; endforeach; endif; ?>
                   </select>
               </div>-->
                <div class="form-group">
                    <label class="form-title">任务时间：</label>
                    <input id="pub_time_1" type="text" name="time_start" value="<?php echo ($task["t_date_start"]); ?>" style="width: 175px;padding:3px"> 至
                    <input id="pub_time_2" type="text" name="time_end" value="<?php echo ($task["t_date_end"]); ?>" style="width: 175px;padding:3px">
                    <label onclick="task()">增加任务</label>
                </div>
                   <?php if(is_array($content)): foreach($content as $k=>$v): ?><div class="form-group">
                           <label class="form-title">任务名称：</label>
                           <input  type="text" name="task_name[]" value="<?php echo ($v["name"]); ?>" placeholder="" style="width:100px;padding:3px"/>
                           <label>时间点:</label>
                           <input type="time" <?php echo (taskIsShow($data["task_heshui"]["s"])); ?> class="task_time" name="start[]" value="<?php echo ($v["time_start"]); ?>">——<input type="time" <?php echo (taskIsShow($data["task_heshui"]["s"])); ?> name="end[]" value="<?php echo ($v["time_end"]); ?>">
...
$(function(){
	h_upload('img_but','/ssyp/index.php/Admin/Upload/Upload','callback_image');
})
var oldImgName = '<?php echo ($data["images"]); ?>';
function callback_image(imgdata){
	...
	oldImgName = imgdata;
	$('#image_show').show().attr('src','/ssyp/Uploads/'+imgdata)
    ...
    function task(){
        var text='<div class="form-group"> ' +
            ...
            '<label>时间点:</label>' +
            '<input type="time" <?php echo (taskIsShow($data["task_heshui"]["s"])); ?> class="task_time" name="start[]" value="">——<input type="time" <?php echo (taskIsShow($data["task_heshui"]["s"])); ?> name="end[]" value="">' +
            '</div>';
