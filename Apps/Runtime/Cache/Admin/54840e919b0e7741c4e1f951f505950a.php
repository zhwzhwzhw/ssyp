	...
	<div class="container">
		...
		<div style="border-bottom: 1px solid #3c8dbc;font-size: 16px;padding-bottom: 10px;" >
			<span style="width: 200px;margin-left: 10px">
				<a style="color: #367FA9" href="/ssyp/index.php/admin//user/index">会员列表</a>
			</span>——>
			<span>任务完成情况</span>
		</div>
		<div style="margin-top: 0" class="detail">
			<a href="/ssyp/index.php/Admin/User/addtask" class="btn btn-large btn-update">添加任务</a>
		</div>
		<div class="table-main">
			<table class="table">
				...
				<?php if(is_array($task)): foreach($task as $key=>$v): ?><tr>
						...
						<td><?php echo ($v["t_username"]); ?></td>
						<td><?php echo (date('Y/m/d',$v["t_date_start"])); ?>-<?php echo (date('Y/m/d',$v["t_date_end"])); ?></td>
						<td>
							<?php if(is_array($v["content"])): foreach($v["content"] as $key=>$val): ?><div>任务名:<?php echo ($val["name"]); ?>&nbsp;&nbsp;时间<?php echo ($val["time_start"]); ?>--<?php echo ($val["time_end"]); ?></div><?php endforeach; endif; ?>
						</td>
						<td>
							<a href="/ssyp/index.php/Admin/User/clock?id=<?php echo ($v["t_id"]); ?>&userid=<?php echo ($user_id); ?>" class="btn btn-large  btn-update">查看</a>
							<a href="/ssyp/index.php/Admin/User/addtask?id=<?php echo ($v["t_id"]); ?>&userid=<?php echo ($user_id); ?>" class="btn btn-large  btn-update">编辑</a>
							<a href="/ssyp/index.php/Admin/User/deltask?id=<?php echo ($v["t_id"]); ?>&userid=<?php echo ($user_id); ?>" class="btn btn-large btn-delete">删除</a>
						</td>
