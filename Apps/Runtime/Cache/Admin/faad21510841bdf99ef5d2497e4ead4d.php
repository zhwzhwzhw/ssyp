...
<form onsubmit="return form_submit(this)" method="post" action="/ssyp/index.php/Admin/Admin/save">
<table id="detail">
	<tr>
	...
	<?php if($useid > 0): ?><tr>
			<th>微信昵称</th>
			<td>
				<input type="text" disabled="disabled"  name="nickName" value="<?php echo ($nickName); ?>">
				<input type="hidden" name="user_id"  value="<?php echo ($useid); ?>">
			</td>
		</tr><?php endif; ?>
	<tr>
		<th>电话号码</th>
		<td><input type="text" name="phone" placeholder="填写电话号码" value="<?php echo ($data["phone"]); ?>"></td>
	</tr>
	<tr>
		<th>真实姓名</th>
		<td><input type="text" name="realname" placeholder="填写真实姓名" value="<?php echo ($data["realname"]); ?>"></td>
	</tr>
	<tr>
		<th>选择商铺</th>
		<td>
			<select name="cat_shop"  onchange="shop_change(this)">
					<option <?php if(($data["role_id"]) == "0"): ?>selected<?php endif; ?> value="0">总店</option>
					<?php if(is_array($shop_list)): foreach($shop_list as $key=>$rv): ?><option <?php if(($rv["id"]) == $data["cat_shop"]): ?>selected<?php endif; ?> value="<?php echo ($rv["id"]); ?>"><?php echo ($rv["seller_name"]); ?></option><?php endforeach; endif; ?>
			</select>
			选择分店时角色请选择 【分店账号】。选择总店时角色 不要选 【分店账号】
		</td>
	</tr>
	...
	<tr>
		<th>选择角色</th>
		<td>
			<select name="role_id"  onchange="role_change(this)">
					<option <?php if(($data["role_id"]) == "1"): ?>selected<?php endif; ?> value="0">暂无权限</option>
					<?php if(is_array($role)): foreach($role as $key=>$rv): ?><option <?php if(($rv["id"]) == $data["role_id"]): ?>selected<?php endif; ?> value="<?php echo ($rv["id"]); ?>"><?php echo ($rv["role_name"]); ?></option><?php endforeach; endif; ?>
					<option <?php if(($data["role_id"]) == "-1"): ?>selected<?php endif; ?> value="-1">总管理员</option>
			</select>
		</td>
	</tr>
...
function shop_change(t){
	if($(t).val() > 0){
		$('[name="role_id"]').val(shop_fendian)
	}
}
function role_change(t){
	if($(t).val() != shop_fendian){
		$('[name="cat_shop"]').val(0)
	}
}
</script>
