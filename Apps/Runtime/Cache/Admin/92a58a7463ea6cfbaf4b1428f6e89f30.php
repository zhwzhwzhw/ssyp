...
<table id="detail" class="list">
	...
	<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
		<td>
			<?php echo ($v["transaction_id"]); ?>
		</td>
		<td>
			<?php echo ($v['money']/100); ?>元
		</td>
		<td>
			<?php echo (date_wx($v["time_end"])); ?>
		</td>
		<td>用户下单</td>
...
function data_num(id,start,end){
	...
	$.ajax({
		'url':'/ssyp/index.php/Admin/Money/dateJson',
		'data':{'id':id},
		'dataType':'json',
		'success':function(re){
...
function click_date_ext(){
	...
	layer.prompt({title: '输 '+data.date+' 的预定限制数量', formType: 3}, function(number, index){
	  layer.close(index);
	  $.ajax({
		'url':'/ssyp/index.php/Admin/Money/dataNum',
		'type':'post',
